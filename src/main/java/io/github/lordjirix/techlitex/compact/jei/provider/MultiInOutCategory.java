...
public class MultiInOutCategory implements IRecipeCategory<MultiOutRecipe> {
  ...
  public MultiInOutCategory(
      IGuiHelper guiHelper, MD.MachineType recipe_type, RecipeType<MultiOutRecipe> type) {
    this.background = guiHelper.createDrawable(TEXTURE, 0, 0, 150, 70);
    this.icon = guiHelper.createDrawableItemStack(new ItemStack(recipe_type.getIcon().get()));
  ...
  @Override
  public void setRecipe(IRecipeLayoutBuilder builder, MultiOutRecipe recipe, IFocusGroup focuses) {

    builder
        .addSlot(RecipeIngredientRole.INPUT, 24, 35)
        .addItemStack(new ItemStack(recipe.getInput(), recipe.getInputCount()));

    for (int i = 0; i < recipe.getOutput().length; i++) {
      builder
          .addSlot(RecipeIngredientRole.OUTPUT, 62 + i * 18, 35)
